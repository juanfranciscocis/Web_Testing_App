...
//		ChromeDriver driver = (ChromeDriver) WebTesting.launchDriver("https://www.google.com", "chrome");
		...
		driver.findElement(By.name("q")).sendKeys("internet speed test by Ookla" + Keys.RETURN);
		System.out.println("Search bar filled.And search performed.");
...
//		// HINT: the button has attribute id="knowledge-verticals-internetspeedtest__test_button"
		try {
			shortWait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(".eKjLze > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1) > a:nth-child(1) > h3:nth-child(2)")));
			System.out.println("Speed test link found.");
		} catch (TimeoutException ex) {
			fail("\"RUN SPEED TEST\" button not found.");
		}

		//click the link
		driver.findElement(By.cssSelector(".eKjLze > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > span:nth-child(1) > a:nth-child(1) > h3:nth-child(2)")).click();
		System.out.println("Speed test link clicked.Opening speed test page.");

		try {
			shortWait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".start-text")));
			System.out.println("Button to run founded.");

		} catch (TimeoutException ex) {
			fail("\" Cant find button\"");
		}
//
//
//
...
//		// #5: TODO (ensure that the "CANCEL" button is clickable directly after this)
		// Because im in ecuador im not able to cancel the test because there is no  button to cancel the test
...
//		// HINT: The "CANCEL" button should not be clickable. Is shortWait long enough?
		try {
			longWait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(".start-text"))); //this is the button to run the test again
		} catch (TimeoutException ex) {
...
//		// HINT: the "RETRY" button is a tag <g-raised-button> with attribute jsaction="i0JLwd"
		try {
			longWait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("html body.standard-layout.language-es div#container div.pre-fold div.main-content div.main div.container div.pure-g div.pure-u-custom-speedtest div.speedtest-container.main-row div.main-view div div.test-status.error div.error-container.notification div.notification-content h2")));
			fail("Speed test failed to run.");
		} catch (TimeoutException ex) {
			System.out.println("Speed test ran successfully.");
		}
//
//
//
